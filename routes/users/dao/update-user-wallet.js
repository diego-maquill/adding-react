...
router.put('/', isLoggedIn, (req, res) => {
	...
	if (Object.keys(amount).length === 0 || amount == undefined) { // check for empty json passed
		...
		res.status(400).json({
			error: "No card details provided"
		});
	...
	usersCardData.getCardByIds(email, cardUsed).then((cardInfo) => {
			...
			usersWalletData.addCash(email, JSON.parse(amount)).then((walletInfo) => {
				if (walletInfo == false) {
					res.json({
						success: false,
						error: "Total wallet limit exceed. Maximum $10000 are allowed."
					})
				} else if (walletInfo) {
...
router.post('/', isLoggedIn, (req, res) => {
	...
	if (Object.keys(amount).length === 0 || amount == undefined) { // check for empty json passed
		res.status(400).json({
			error: "No wallet details provided provided"
		});
	} else if (!amount) {
		res.status(400).json({
			error: "No amount provided"
